    ...
    if x > 128:
        ...
        if x > 128:
            ...
            if x > 128:
                ...
                strn = str(int(n))
                m = (n - int(strn)) * 128
                return dec_list[int(m)] + dec_list[int(n)] + dec_list[int(z)] + dec_list[int(y)] + x_list[int(x)]
            else:
                strx = int(x)
                y = (x - int(strx)) * 128
                stry = str(int(y))
                ...
                n = (z - int(strz)) * 128
                strn = str(int(n))
                return dec_list[int(n)] + dec_list[int(z)] + dec_list[int(y)] + x_list[int(x)]
...
def remove_friend_single(uid, token, region="IND"):
    try:
        encrypted_id = Encrypt_ID(uid)
        if not encrypted_id:
            print(f"❌ Failed to encrypt UID {uid}")
        ...
        if region.upper() in ("IND", "IN"):
            ...
            url = "https://client.bd.freefiremobile.com/RemoveFriend"
        else:
            url = "https://client.ind.freefiremobile.com/RemoveFriend"
        ...
        print(f"📤 Removing friend {uid}...")
        response = requests.post(url, data=bytes.fromhex(encrypted_payload), headers=headers, timeout=10, verify=False)

        if response.status_code == 200:
            print(f"✅ Success: Friend {uid} removed")
            return True
        else:
            print(f"❌ Failed: Status {response.status_code}")
            return False
...
async def GeNeRaTeAccEss(uid, password):
    ...
    try:
        ...
        url = f"https://jwt.bhimsainik.qzz.io/token?uid={uid}&password={encoded_password}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=24) as response:
                if response.status == 200:
                    ...
                    return data.get("open_id"), data.get("token_access")
                return None, None
    except:
        return None, None
...
async def api_start(request):
    ...
    team_code = request.query.get('team')
    if not team_code:
        return web.json_response({"error": "Missing team parameter"}, status=400)
    
    if auto_start_running:
        return web.json_response({"error": "Auto start is already running"}, status=400)
        
    if not global_key or not global_iv:
        return web.json_response({"error": "Bot is not fully connected to Free Fire yet"}, status=503)
        
    stop_auto = False
    auto_start_running = True
    auto_start_task = asyncio.create_task(auto_start_loop(team_code, 0, 0, 0, global_key, global_iv, global_region))
    return web.json_response({"status": "success", "message": f"Auto level up started for team {team_code}"})

async def api_stop(request):
    await stop_auto_loop()
    return web.json_response({"status": "success", "message": "Auto level up stopped"})

async def api_status(request):
    return web.json_response({
        "running": auto_start_running,
        "connected": global_key is not None
    })

async def start_web_server():
    app = web.Application()
    app.router.add_get('/start', api_start)
    app.router.add_get('/stop', api_stop)
    ...
    await site.start()
    print("🌐 Discord Web API Server listening on port 5000")
...
async def MajorLogin(payload):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=payload, headers=Hr, ssl=ssl_context) as response:
            ...
            print(f"MajorLogin failed with HTTP {response.status}: {err_text}")
            return None
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    try:
                        ...
                        if inPuTMsG.startswith('/lw '):
                        ...
                        elif inPuTMsG.startswith('/add '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]Usage: /add <uid>", uid, chat_id, key, iv)
                                continue
                            target_uid = parts[1]
                            if not target_uid.isdigit():
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]UID must be numbers only", uid, chat_id, key, iv)
                                continue
                            await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]📤 Sending friend request to {target_uid}...", uid, chat_id, key, iv)
                            try:
                                with open("token.json", "r") as tf:
                                    token_data = json.load(tf)
                                bot_token = token_data.get("token")
                                bot_region = token_data.get("region", "IND")
                                if not bot_token:
                                    await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ Bot token not found", uid, chat_id, key, iv)
                                    continue
                                success = send_friend_request_single(target_uid, bot_token, bot_region)
                                if success:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][00FF00]✅ Friend request sent to {target_uid}", uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][FF0000]❌ Failed to send friend request to {target_uid}", uid, chat_id, key, iv)
                            except Exception as e:
                                print(f"Add friend error: {e}")
                                await safe_send_message(response.Data.chat_type, f"[B][C][FF0000]❌ Error: {str(e)[:50]}", uid, chat_id, key, iv)

                        elif inPuTMsG.startswith('/remove '):
                            parts = inPuTMsG.strip().split()
                            if len(parts) < 2:
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]Usage: /remove <uid>", uid, chat_id, key, iv)
                                continue
                            target_uid = parts[1]
                            if not target_uid.isdigit():
                                await safe_send_message(response.Data.chat_type, "[B][C][FF0000]UID must be numbers only", uid, chat_id, key, iv)
                                continue
                            await safe_send_message(response.Data.chat_type, f"[B][C][FFFF00]📤 Removing friend {target_uid}...", uid, chat_id, key, iv)
                            try:
                                with open("token.json", "r") as tf:
                                    token_data = json.load(tf)
                                bot_token = token_data.get("token")
                                ...
                                if not bot_token:
                                    await safe_send_message(response.Data.chat_type, "[B][C][FF0000]❌ Bot token not found", uid, chat_id, key, iv)
                                    continue
                                success = remove_friend_single(target_uid, bot_token, bot_region)
                                if success:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][00FF00]✅ Friend {target_uid} removed", uid, chat_id, key, iv)
                                else:
                                    await safe_send_message(response.Data.chat_type, f"[B][C][FF0000]❌ Failed to remove friend {target_uid}", uid, chat_id, key, iv)
                            except Exception as e:
                            ...
                            help_msg = (
                                ...
                                "[FFFFFF]/add <uid>       [00FF00]- Add friend to bot\n"
                                "[FFFFFF]/remove <uid>    [00FF00]- Remove friend from bot\n"
...
async def MaiiiinE():
    ...
    global global_key, global_iv, global_region
    global_key = key
    global_iv = iv
    ...
    online_ip, online_port = ports.Online_IP_Port.rsplit(":", 1)
    chat_ip, chat_port = ports.AccountIP_Port.rsplit(":", 1)
